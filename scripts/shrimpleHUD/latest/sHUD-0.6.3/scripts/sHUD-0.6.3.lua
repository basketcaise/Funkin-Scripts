...
local hudFunctions = {
	[1] = function() -- Vanilla-like
		...
		if not luaTextExists('vScoreTxt') then
			makeLuaText('vScoreTxt', 'SCORE: '..score..' ['..ratingFC..']', 200, getProperty('healthBar.x') + 397.5, getProperty('healthBar.y') - 25)
		...
		setProperty('timeBar.y', getProperty('healthBar.y') + 22.5)
		setProperty('timeTxt.y', getProperty('healthBar.y') + 18)
	...
	[2] = function() -- Shrimple
		...
		if downscroll then
			...
			setProperty('sScoreTxt.y', getProperty('healthBar.y') + 17.5)
			...
			if botPlay then
				setProperty('sScoreTxt.y', getProperty('healthBar.y') - 25)
			else
				setProperty('sScoreTxt.y', getProperty('healthBar.y') - 55)
			end
	...
	[3] = function() -- sKade
		...
		if downscroll then
			...
			setProperty('kWatermark.y',  getProperty('timeBar.y') + 5)
		else
		...
		setProperty('timeBar.x', getProperty('timeBar.x'))
		setProperty('timeTxt.y', getProperty('timeBar.y') - 5)
...
function onCreatePost()
	if not disableShrimpHUD then
		if version ~= '0.6.3' then
			debugPrint('This version of shrimpleHUD is intended for v0.6.3. Things might not work correctly!')
		end
...
function onUpdateScore()
	...
	if not botPlay and not disableShrimpHUD and startedCountdown then
		scoreFunctions[getDataFromSave('shrimpleHUD', 'HUD')]()
	end
end

function onEvent(n)
	if curHUD == 3 and not disableShrimpHUD and n == 'Change Character' then
		setHealthBarColors('FF0000', '66FF33')
	end
end
